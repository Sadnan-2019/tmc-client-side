...
const Business = () => {
  ...
  useEffect(() => {
    ...
    // Use ScrollSmoother for smoother scrolling
    ScrollTrigger.config({
      limitCallbacks: true,
    });

    return () => {
      // Ensure to kill the ScrollTrigger instance on component unmount
      ScrollTrigger.getById('boxTrigger')?.kill();
    };
  }, []);
  useGSAP(
     () => {
     ...
     return (
          <div ref={main}  id="smooth-wrapper" className="bg-gradient-to-r from-[#157A90] via-[#160a2c] to-[#157A90]">
     


    <div id="smooth-content" className=" scroll-container grid grid-cols-1 lg:grid-cols-3 gap-5 px-5 mx-5 justify-center items-center " >
